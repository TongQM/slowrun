...
All curves are evaluated on $20$ independently-trained $d12\_w768$ GPT models on a $df=0.2$ FineWeb subset (Q2 setup), with constant learning rate and weight decay $0.1$.
We treat $E=1$ as the special case of an $E$-member ensemble with $E=1$, taking the per-snapshot mean across the $20$ individuals as the $E=1$ datum, and fit
$$L_{T,E} \;=\; a_T^{(\text{strat})}\,E^{-b^{(\text{strat})}} + L_{\infty,T}$$
jointly across both strategies, all $40$ epoch-boundary snapshots $T \in \{1, \dots, 40\}$, and $E \in \{1, 2, 5, 10, 15, 20\}$.
...
The fit is a joint nonlinear least-squares optimization with $480$ observations ($6$ ensemble sizes $\times\, 40$ snapshots $\times\, 2$ strategies) and $122$ free parameters ($2$ exponents, $2 \times 40$ prefactors, $40$ shared asymptotes).
...
Fitted exponents (mean $\pm$ SE) are $b = 0.88 \pm 0.01$ for init only and $b = 1.09 \pm 0.01$ for init plus shuffle.
Wald (z) tests reject all three natural null hypotheses with extreme significance: $b_{\mathrm{init}} = 1$ ($z = -11.8$, $p < 10^{-30}$), $b_{\mathrm{shuffle}} = 1$ ($z = +6.6$, $p = 3.5 \times 10^{-11}$), and $b_{\mathrm{init}} = b_{\mathrm{shuffle}}$ ($\Delta b = +0.21 \pm 0.02$, $z = +12.3$, $p < 10^{-30}$).
The prefactors $a_T$ themselves obey a power law in cumulative tokens $T$: $a_T \approx (5.4\times 10^{-7})\,T^{0.69 \pm 0.02}$ for init and $a_T \approx (1.8\times 10^{-6})\,T^{0.63 \pm 0.03}$ for init plus shuffle.
...
Init randomness alone yields $b = 0.88 \pm 0.01$, so the loss-gap decays \emph{slower} than $1/E$, which is consistent with partially correlated members; the same data ordering across epochs limits how much individual seeds diverge.
Adding independent per-epoch data permutations (init plus shuffle) yields $b = 1.09 \pm 0.01$, so the loss-gap decays \emph{faster} than $1/E$, which means each additional member contributes more than a $1/E$ share of variance reduction.
The two strategies thus bracket the naive $b = 1$ prior that assumes fully independent members; both deviations from the prior are statistically significant ($p < 10^{-10}$, Wald tests on the joint fit), as is the gap between the two strategies' exponents ($p < 10^{-30}$).
Figure~\ref{fig:ensemble_scaling} (right) shows that the prefactor itself grows as a power of training time, $a_T \propto T^{p}$ with $p \approx 0.6$ to $0.7$, which means the absolute benefit of ensembling \emph{increases} with how long each individual is trained.
