#!/bin/bash
# Q1: Data-size ablation on d12/w768.
# 4 dfs × 2 individuals (1 init_ens + 1 init_shuffle_ens) × 50 epochs.
# Goal: see overfit-onset epoch as function of data fraction.
#
# Constant LR + CompleteP + no-ve-projs throughout.
...
GRID_TAG="${GRID_TAG:-q1_$(date +%Y%m%d_%H%M%S)}"
DRY_RUN="${DRY_RUN:-0}"
...
declare -a DFS=(0.2 0.4 0.6 0.8)
L=12; H=12; W=768

echo "============================================================"
echo "Q1 data-size sweep: d${L}/w${W}, dfs=${DFS[*]}"
echo "  NUM_EPOCHS=$NUM_EPOCHS, 1 ind per strategy, 2 strats per cell"
...
submit_cell() {
    ...
    exports+=",ENS_SIZES_STR=2,SKIP_INDIV_VAL=1,END_EPOCH=$NUM_EPOCHS"

    # Time scales linearly with df. df=0.8 50 epochs ~6 h; pad to 10 h.
    local timelim
    case "$df" in
        0.2) timelim="04:00:00" ;;
        0.4) timelim="06:00:00" ;;
        0.6) timelim="08:00:00" ;;
        0.8) timelim="10:00:00" ;;
        *)   timelim="12:00:00" ;;
    esac

    # Train: 2 tasks (init_ens model 0 + init_shuffle_ens model 0)
    local TJOB
    TJOB=$(submit_one "q1_train_d12_w768_df${df}" "" "$timelim" "0-1" "$exports" experiments/parallel/train_array.sh)
    echo "  df=$df  train  job=$TJOB  timelim=$timelim  group=$GROUP" >&2

    # Cleanup: 2 tasks (one per strategy)
    local CJOB
    CJOB=$(submit_one "q1_cleanup_d12_w768_df${df}" "--dependency=afterok:$TJOB" "00:30:00" "0-1" "$exports" experiments/parallel/cleanup_array.sh)
...
echo
echo "Q1 submission complete. Wandb groups: q1_data_size_${GRID_TAG}_d12_w768_df{0.2,0.4,0.6,0.8}"
